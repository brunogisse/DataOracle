...
begin
      ...
      if editPesquisaNF.Text = '' then
         ...
         Except
             on E : Exception do
                MessageDlg('Informe um período válido',TMsgDlgType.mtWarning,[TMsgDlgBtn.mbOK],0);
         end
            else
              with qryPagarParcelas do
                  begin
...
begin
    if (qryPagarParcelas.RecordCount > 0) then
               begin
                  if MessageBox(Application.Handle,'Confirma o pagamento da parcela?','Confirmação', MB_YESNO + MB_ICONQUESTION) = mrYes  then
                    try
                    ...
                    on E : exception do
                      begin
                        ...
                        MessageDlg('Erro ao pagar a parcela. ' + #13 + #13 +
                                   'Cheque se a data de pagamento é válida ou se o problema é diferente da indicação técnica abaixo: ' + #13 + #13 +
                                   'Erro Técnico: '+#13+#13 + AuxError,TMsgDlgType.mtWarning,[TMsgDlgBtn.mbOK],0);
                      end;
                    end;
               end
            else
              MessageDlg('Não há parcelas a serem pagas'+#13+#13,TMsgDlgType.mtWarning,[TMsgDlgBtn.mbOK],0);
end;
...
begin
     ...
     // identifcação de qual venda se trata. Importante para editar os valores de vendas e também para dar baixa
     // no estoque da usina correta, localizada na venda específica....
     ...
     // Atualização na tabela das parcelas, identificando o status como pago e registrando a data de pagamento...
     ...
     // Identificando a usina na tabela "ESTOQUE_USINA" através da tabela "VENDA_POSTOS"...
        ...
        //Registrando a movimentação de saída do estoque mediante o pagamento da parcela.
        //Está neste trecho para captura a ID da tabela "Estoque_usina" que está logo acima...
        RegistrarMovimentoEstoque('salvando');

        qryEstoqueUsina.Post;


       {editando a tabela vendas, ajustando o valor recebido até o momento. Sequência:
        1 - captura o mes na variável "CompararMes" para comparação do mes de recebimento do valor.
        2 - se for o mesmo mes (ex: duas parcelas recebidas no mesmo mês), o "VALOR_RECEBIDO_MES"
            recebe a soma da parcelas. Senão, substitui o valor pela parcela daquele mês.}
            qryVendas.Edit;
      ...
      // 1 Recebe o valor da parcela e vai somando ao total recebido no momento, até que seja o valor original da NF
      // 2 Faz a mesma coisa com o volume
      // 3 Atualiza a data de vencimento para ser usada como parâmetro no relatório de fechamento
      // 4 bloqueia a venda para que não seja possível fazer alterações.
