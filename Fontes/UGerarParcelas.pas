...
uses
   ...
   Vcl.DBCtrls, DateUtils, Vcl.Mask, System.ImageList, Vcl.ImgList;
...
type
   ...
   TfrmGerarParcelas = class(TForm)
      ...
      procedure gridGerarParcelaDrawColumnCell(Sender: TObject;
      ...
      procedure gridGerarParcelaTitleClick(Column: TColumn);
   private
...
uses UVendaPosto, UPrincipalPetrotorque, UEditarParcelas, UFuncoes;
...
begin
   ...
   TdbGridPadrao(gridGerarParcela).ClientHeight :=
     (25 * TdbGridPadrao(gridGerarParcela).RowCount) + 25;
...
begin
   ...
   qryParcelas.ParamByName('vendaid').AsInteger := frmVendaPostos.qryVendaPosto
     ['VENDAID'];
   qryParcelas.Open();
...
begin
   ...
   gridGerarParcela.Canvas.TextRect(Rect, Rect.Left + 8, Rect.Top + 8,
...
procedure TfrmGerarParcelas.gridGerarParcelaTitleClick(Column: TColumn);
begin
...
begin
   ...
   begin
      ...
      qryParcelas['PARCELA'] := IntToStr(I) + '/' + IntToStr(qtdParcelas);
      // registra parcela atual. Ex: "1/3 ... 2/3 e 3/3".
      ...
      qryParcelas['EMISSAO_NF'] := frmVendaPostos.qryVendaPosto
        ['DATA_EMISSAO_NF'];
      ...
      DataParcela := IncMonth(DataParcela);
      // função que incrementa um mês à parcela.

      qryParcelas['VOLUME_VENDA_TOTAL'] := frmVendaPostos.qryVendaPosto
        ['VOLUME'];
