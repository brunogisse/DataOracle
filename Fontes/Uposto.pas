...
uses
  ...
  System.Classes, Vcl.Graphics, MaskUtils,
...
begin
  ...
  cbMunicipio.KeyValue := cbMunicipio.ListSource.DataSet.FieldByName
    (cbMunicipio.KeyField).Value;
end;
...
begin
  if not(qryPosto.State in [dsEdit, dsInsert]) then
  begin
...
begin
  ...
  TdbGridPadrao(gridPosto).ClientHeight :=
    (25 * TdbGridPadrao(gridPosto).RowCount) + 25;
...
begin
  configurarEnables(0);
  gbPesquisaPosto.Enabled := True;
  if MessageBox(Application.Handle, 'Deseja excluir esse registro?',
    ...
    try
      ...
      begin
        ...
        MessageDlg('Não foi possível excluir esse registro. ' + #13 + #13 +
          'Esse cliente está sendo referenciado em alguma venda.  ' + #13 + #13
          + 'Erro Técnico: ' + AuxErro, TMsgDlgType.mtWarning,
          [TMsgDlgBtn.mbOK], 0);
      end;
    end;
...
begin
  ...
  if (qryPosto.RecordCount > 0) and (DuploClickNaGrid = 'relatoriofechamento')
  then
  begin
  ...
  if rbCNPJ.Checked = True then
    ...
    begin
      ...
      SQL.Add('select p.postoid, p.nome_fantasia, p.razao_social, p.cnpj,  p.estado_id, p.municipio_id, e.uf, m.nome '
        + ' from Posto p, estado e, municipio m ' + ' where ' +
        '(p.estado_id = e.id) and ' + '(p.municipio_id = m.id) and cnpj like ' +
        QuotedStr('%' + editPesquisaPosto.Text + '%') + 'order by cnpj asc');
      Open;
    end;
  ...
  if rbNomeFantasia.Checked = True then
    ...
    begin
      ...
      SQL.Add('select p.postoid, p.nome_fantasia, p.razao_social, p.cnpj,  p.estado_id, p.municipio_id, e.uf, m.nome '
        ...
        '(p.municipio_id = m.id) and nome_fantasia like ' +
        QuotedStr('%' + editPesquisaPosto.Text + '%') +
  ...
  if rbRazaoSocial.Checked = True then
    ...
    begin
      ...
      SQL.Add('select p.postoid, p.nome_fantasia, p.razao_social, p.cnpj,  p.estado_id, p.municipio_id, e.uf, m.nome '
        + ' from Posto p, estado e, municipio m ' + ' where ' +
        '(p.estado_id = e.id) and ' +
        '(p.municipio_id = m.id) and razao_social like ' +
        QuotedStr('%' + editPesquisaPosto.Text + '%') +
