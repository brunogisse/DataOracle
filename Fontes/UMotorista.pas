...
type
   ...
   public
      ...
      procedure proc_EditsExibemNomesSelecionadosNaGrid(qry: TFDQuery);
...
procedure TfrmMotorista.proc_EditsExibemNomesSelecionadosNaGrid(qry: TFDQuery);
begin
   if qry.State in [dsEdit] then
      Exit;

   if qry['NOME'] <> null then
...
begin
   if Assigned(qryObjeto) then
      proc_EditsExibemNomesSelecionadosNaGrid(qryObjeto)
   else
      proc_EditsExibemNomesSelecionadosNaGrid(qryMotorista);
end;
